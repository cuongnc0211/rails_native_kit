...
| Page looks "zoomed in" / right edge of nav cut off | Horizontal overflow a few px wider than viewport — clipping, not zoom | Fix the wide element or `overflow-x-hidden` on `<body>`; server-side, users just pull-to-refresh |
